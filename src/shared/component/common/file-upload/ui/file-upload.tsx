...
import {useUploadFilesId} from "@shared/component/common/file-upload/api/use-upload-files-id.ts";
...
function InputFileComponent<T extends FieldValues>({
  ...
  const { mutate: mutateById, isPending: isPendingById } = useUploadFilesId({
    endpoint: uploadEndpoint,
    onUploadProgress: handleUploadProgress,
  });
        ...
        mutateById([file], {
          onSuccess: (data) => {
            ...
            if (data && data.file.length > 0) {
              setValue(name, data.id as PathValue<T, Path<T>>, { shouldValidate: true });
