...
import { ProjectDetail, ProjectForm } from "@interfaces/projects.interface";
import useEditProject from "@modules/projects/hooks/useEditProject";
...
export default function EditProject() {
  ...
  const useEditProjectData = useEditProject();
  async function onSubmit(data: ProjectForm) {
    const res = await useEditProjectData.addProject(data);
    if(res){
  ...
  useEffect(() => {
    if (data) {
      reset({
        name: data.name,
        type: data.type?.id,
        owner_organization: data.owner_organization?.id,
        district: data.district?.map((d) => d.id),
        address: data.address,
        used: data.used,
        investment_plan_created: data.investment_plan_created,
      });
    }
  }, [data, reset]);
