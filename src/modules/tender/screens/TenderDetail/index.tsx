...
const Index = () => {
  ...
  const { isPending: isOperatorApprovePending, operatorApproveAppeal } =
    useOperatorApprove();
  ...
  useEffect(() => {
    if (!data) return;
    const totalBall =
      data.investment_ability_ball +
      data.region_by_ball +
      data.experience_ball +
      data.techniques_ball +
      data.stability_rating_ball +
      data.success_projects_ball +
      data.return_on_investment_ball;
      ...
      },
      {
        ...
        accessor: (row) => {
          if (row.evaluation_status) {
            ...
            typeof row.evaluation_status === "boolean"
          ) {
        ...
        accessor: (row) => {
          if (row.evaluation_text) {
            return (
              <Button
                mini
                onClick={() => setPerformerRejectText(row.evaluation_text)}
              >
                {t("show")}
              </Button>
            );
  ...
  const IsReadyToConcept =
    data &&
    data.confirmation_documents.length > 0 &&
    data.evaluation_confirmation_documents;

  const IsReadyToConfirmDocumentApplicationEvolution =
    data &&
    data.evaluation_confirmation_documents === null &&
    data.confirmation_documents.length > 0;
  ...
  useEffect(() => {
    ...
    const performer = data.performers.find(
      (item) => item.performer.id === user.id,
    );
    ...
    const allFieldsTrue = data.performers.every(
    ...
    <PageLayout>
      ...
      <Wrapper>
          ...
          {[
            ...
            <>
              <Tag title="Reply letter">
                {
                  <Status
                    status={data?.answer_type ?? STATUS_LIST.INCOMPLETE}
                  />
                }
              </Tag>
              <Tag title="Type of request">
                {data?.type && <Status status={data.type} />}
              </Tag>
            </>
          )}
        ...
        <div className="grid grid--cols-2  gap--lg">
          ...
          <GridWrapper>
            ...
            <Row
              ...
              value={data?.balance_organization?.name}
              background
            />
        ...
        <div className="grid grid--cols-2  gap--lg">
          <GridWrapper>
            <Row
              label="appeals.successful_projects_text"
              value={data?.success_projects}
              background
            />
            <Row
              label="applications.investment_ability_ball"
              value={data?.investment_ability_ball}
            />
            <Row
              label="applications.experience_ball"
              value={data?.experience_ball}
            />
            <Row
              label="appeals.project_financing_opportunity"
              value={data?.financing_opportunity}
              background
            />
            {data?.return_on_investment ? (
              <Row label="investment_type" value={t("Yes")} background />
            ) : (
              <Row label="investment_type" value={t("No")} background />
            )}
          </GridWrapper>

          <GridWrapper>
            <Row
              label="applications.techniques_ball"
              value={data?.techniques_ball}
              background
            />
            <Row
              label="applications.stability_rating_ball"
              value={data?.stability_rating_ball}
            />
            <Row
              label="applications.success_projects_ball"
              value={data?.success_projects_ball}
              background
            />
            <Row
              label="applications.return_on_investment_ball"
              value={data?.return_on_investment_ball}
            />
            <Row
              label="applications.region_by_ball"
              value={data?.region_by_ball}
            />
          ...
          <GridWrapper>
            <Row
              label="applications.application_content"
              value={data?.body}
              background
            />
          </GridWrapper>
        ...
        <div className={"mb-5"}>
          <h2 className={"font-semibold text-gray-400 mb-3"}>
            {t("applications.balance_organization_relationship")}
          </h2>

          {data && data?.object_evaluation_balance_organization === null ? (
            <div>not selected</div>
          ) : typeof data?.object_evaluation_balance_organization ===
              "boolean" && data?.object_evaluation_balance_organization ? (
            <div
              className={
                "bg-red-100 rounded-[50px] px-5 py-2 max-w-[200px] text-center text-red-500 font-semibold"
              }
            >
            ...
            <div
              className={
                "bg-green-100  rounded-[50px] px-5 py-2 max-w-[200px] text-center text-green-700 font-semibold"
              }
            >
          ...
          {data?.object_evaluation_balance_organization_text ? (
            <div className={"pt-5"}>
              <div className={"font-semibold text-gray-400 mb-3"}>
                Band munosobati:
              </div>
              <div>{data.object_evaluation_balance_organization_text}</div>
            </div>
          ) : null}
          {data?.rejection_file ? (
            <div className={"pt-5 flex justify-between max-w-[300px]"}>
              ...
              <div>
                <Link className={"text-blue-400"} href={data.rejection_file}>
                  {t("applications.rejection_file")}
                </Link>
              </div>
            </div>
          ) : null}
        ...
        <div className="grid grid-cols-12 gap-5">
          <div className="col-span-6 mb-5">
            <div
              className={"flex item-center justify-between border-b pb-1 mb-5"}
            >
              ...
              <div className={"text-black font-bold pl-3"}>
                {data &&
                data.indebtedness_file &&
                data.indebtedness_file.file ? (
                  <Link
                    className={"text-blue-400"}
                    href={data.indebtedness_file.file}
                  >
                    {data.indebtedness_file.name}
                  </Link>
                ) : null}
              </div>
            </div>
            ...
            >
              ...
              <div className={"text-black font-bold pl-3"}>
                {data &&
                data.success_projects_file &&
                data.success_projects_file.file ? (
                  <Link
                    className={"text-blue-400"}
                    href={data.success_projects_file.file}
                  >
                    {data.success_projects_file.name}
                  </Link>
                ) : null}
              </div>
            </div>
            ...
            <div className={"flex item-center justify-between border-b pb-1"}>
              ...
              <div className={"text-black font-bold pl-3"}>
                {data && data.file ? (
                  <Link className={"text-blue-400"} href={data.file}>
                    {t("applications.file")}
                  </Link>
                ) : null}
              </div>
        ...
        <div className="flex gap--lg items-center justify-between">
            ...
            <div className={"flex justify-end"}>
              ...
              >
                <div className="flex gap--lg items-center mr-3">
                  <Button
                    onClick={() => navigate("balance-organization")}
              ...
              <ShowIf
                show={
                  data?.object_evaluation_balance_organization !== null &&
                  !data?.confirmation_documents.length
                }
              >
                <Button
                  onClick={() => navigate("balance-object-info")}
                  theme={BUTTON_THEME.PRIMARY}
                >
            ...
            <ShowIf show={data?.status === STATUS_LIST.IN_PROCES}>
              <div className="flex gap--lg items-center">
                ...
                {data && !data.performers?.length ? (
                  <Button
                    onClick={() =>
                      navigate(`/applications/${data?.id}/responsibility`)
                    }
                    theme={BUTTON_THEME.PRIMARY}
                  >
                    Set Responsibility
          ...
          <Restricted permittedRole={[ROLE_LIST.MINISTRY_DXSH_B_X]}>
            ...
            >
              <div className="flex gap--lg items-center">
                {!confirmPerformer?.evaluation_status &&
                confirmPerformer?.status === "completed" ? (
                  <Button theme={BUTTON_THEME.DANGER_OUTLINE}>Returned</Button>
                ) : confirmPerformer?.status !== "completed" ? (
                  <Button
                    theme={BUTTON_THEME.DANGER_OUTLINE}
                    onClick={() => navigate("reject-performer")}
                  >
                    Return
                  </Button>
                ) : null}
                ...
                {confirmPerformer?.evaluation_status &&
                confirmPerformer?.status === "completed" ? (
                  <Button theme={BUTTON_THEME.GREEN}>Confirmed</Button>
                ) : confirmPerformer?.status !== "completed" ? (
                  <Button
                    onClick={() => confirmByPerformer()}
                    theme={BUTTON_THEME.PRIMARY}
                  >
                    Confirm
                  </Button>
          ...
          <Restricted permittedRole={[ROLE_LIST.APPLICANT]}>
            <ShowIf show={!!IsReadyToConcept}>
              <div className="flex gap--lg items-center">
                <Button
                  onClick={() => navigate("set-object-concept")}
                  theme={BUTTON_THEME.PRIMARY}
                >
                  set_object_concept
                </Button>
          ...
          <Restricted permittedRole={[ROLE_LIST.BASIN_B_B]}>
            <ShowIf show={!!IsReadyToConfirmDocumentApplicationEvolution}>
              <div className="flex gap--lg items-center">
                <Button
                  onClick={() => confirmByHavzaApplication(false)}
                  theme={BUTTON_THEME.DANGER_OUTLINE}
                >
                  Return
                </Button>
                ...
                <Button
                  onClick={() => confirmByHavzaApplication(true)}
                  theme={BUTTON_THEME.PRIMARY}
                >
                  Confirm
                </Button>
        ...
        {data?.status && data?.answers?.length > 0 && (
          <>
              ...
              >
                ...
                <Tag title="Documents" type="vertical">
                  <div className="grid grid--cols-3 gap--lg">
                    {data?.answers?.[data?.answers?.length - 1]?.files?.map(
                      (file) => {
                        return (
                          <FileUpLoader
                            key={file.id}
            ...
            <Restricted permittedRole={[ROLE_LIST.HEAD, ROLE_LIST.OPERATOR]}>
              ...
              >
                <Tag title="Reply letter" string type="vertical">
                  {data?.answers?.[data?.answers?.length - 1]?.text ?? ""}
                </Tag>
              ...
              >
                <Tag title="Manager's return comment" string type="vertical">
