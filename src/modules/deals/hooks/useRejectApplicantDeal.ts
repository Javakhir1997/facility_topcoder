...
const useRejectApplicantDeal = () => {
    ...
    const {isPending: isRejectPending, mutate: rejectApplicantDeal} = useMutation({
        mutationFn: (data: RejectDealDTO) => apiClient.patch(`${API_ENDPOINTS.DEAL_REJECT_APPLICANT_DXSH}/${id}`, data),
        onSuccess: () => {
    ...
    return {form, rejectApplicantDeal, isRejectPending}
};

export default useRejectApplicantDeal;
